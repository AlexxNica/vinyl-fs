...
function symlink(outFolder, opt) {
  ...
  function linkFile(file, enc, callback) {
    ...
    var useJunctions = options.get(file, 'useJunctions');

    var symType = options.get(file, 'symType');
    var isRelative = options.get(file, 'isRelative');
  ...
  function resolveOptions(file) {
    ...
    var juncDefault = (isWindows && isDirectory);
    ourOpts.useJunctions = koalas(boolean(opt.useJunctions, file), juncDefault);

    var symDirType =  ourOpts.useJunctions ? 'junction' : 'dir';
    var symType = isDirectory ? symDirType : 'file';
    ourOpts.symType = symType;

    ourOpts.isRelative = koalas(boolean(opt.relative, file), false);
